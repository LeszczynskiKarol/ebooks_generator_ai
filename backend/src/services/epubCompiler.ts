...
export async function compileEpub(projectId: string): Promise<{
  ...
  try {
    ...
    // ── 3. Title page ──
    const titleXhtml = generateTitlePage(bookTitle, bookLang);
    fs.writeFileSync(
      path.join(epubDir, "OEBPS", "title.xhtml"),
      titleXhtml,
      "utf-8",
    );
...
function generateTitlePage(title: string, lang: string): string {
